...
function save(){
	...
	var valuename=document.getElementById("input2").value;
	 if (EmbarkJS.isNewWeb3()) {
      SimpleStorage.methods.set(value,valuename).send({from: web3.eth.defaultAccount});
      addToLog("#blockchain", "SimpleStorage.methods.set(value).send({from: web3.eth.defaultAccount})");
    } else {
	  SimpleStorage.set(value,valuename);
...
function Getvalue(){
	 if (EmbarkJS.isNewWeb3()) {
      SimpleStorage.methods.get().call(function(err, value) {
		...
		document.getElementById("valueget").innerHTML=value;
		
      });
	   SimpleStorage.methods.getname().call(function(err, value) {
        // $("#blockchain .value").html(value);
